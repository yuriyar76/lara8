...
<div id="content-single" class="content group">
       ...
       <div class="hentry hentry-post blog-big group ">
        ...
        <div class="meta group">
            ...
            <p class="categories"><span>In:
                    <a href="{{ route('articlesCat', ['cat_alias', $article->category->alias]) }}" title="View all posts in {{ $article->category->title }}" rel="category tag">
                        {{ $article->category->title }}
                    </a>

                </span>
            </p>
            <p class="comments"><span><a href="#comments" title="comments">
                        {{ count($article->comment) ? (count($article->comment)) : '0' }} {{ Lang::choice('ru.comments',  count($article->comment) ? count($article->comment) : '0') }}
                    </a></span></p>
        </div>
        <!-- post content -->
    ...
    <div id="comments">
        <h3 id="comments-title">
            <span>{{ count($article->comment) ? (count($article->comment)) : '0' }}</span> {{ Lang::choice('ru.comments',  count($article->comment) ? count($article->comment) : '0') }}
        </h3>
        ...
        <ol class="commentlist group">
            @foreach($com as $k=>$comments)
                @if($k !== 0)
                    @break
                @endif
                @include(env('THEME').'.include.comment', ['items'=>$comments])
            @endforeach
        </ol>
        @endif
